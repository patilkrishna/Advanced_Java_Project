...
public class EmployeeDaoImple implements EmployeeDao {
	...
	public void deleteExpense(int expenseId) {
		hibernateTemplate.execute(new HibernateCallback<Void>() {
			...
			public Void doInHibernate(Session session) throws HibernateException {
				Transaction tr = session.beginTransaction();
				session.delete(new Employee(expenseId));
				tr.commit();
				session.flush();
				session.close();
				return null;
	...
	@Override
	public Employee selectExpenxe(int expenseId) {
		Employee employee = hibernateTemplate.execute(new HibernateCallback<Employee>() {
			...
			public Employee doInHibernate(Session session) throws HibernateException {
				Transaction tr = session.beginTransaction();
				Employee ex = (Employee)session.get(Employee.class, expenseId);
				tr.commit();
				session.flush();
				session.close();
				return ex;
...
//				Employee ex = (Employee)session.get(Employee.class, expense.getExpenseId());
...
//				ex.setPurchaseDate(expense.getPurchaseDate()); 
	...
	public List<Employee> selectAll(int userId) {
		List<Employee> expList = hibernateTemplate.execute(new HibernateCallback<List<Employee>>() {
